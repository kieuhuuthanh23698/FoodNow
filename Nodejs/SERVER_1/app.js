...
//Login Đăng nhập//
//route post Dangnhap_cuahang
//method post(truyền Tentaikhoan, MatKhau)
//Param  Tentaikhoan, MatKhau
app.post("/Dangnhap_cuahang", urlEncodeParser, function (req, res) {
	CUAHANG.aggregate(
		[
			{
				"$match": {
					"Ten_Tai_Khoan": mongoose.Types.ObjectId(req.body.Tentaikhoan),
					"Mat_Khau": mongoose.Types.ObjectId(req.body.MatKhau)
				}
			},
			{
				"$project": {
		...
		function (err, result) {
			if (err)
				...
				res.send("Lấy tài khoản cửa hàng thành công : " + result);
			}
		}
	);
